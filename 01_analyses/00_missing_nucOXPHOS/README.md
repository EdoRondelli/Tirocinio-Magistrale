...
```
##The first search was seeing if and how many matches were found between the proteins and the FASTA headers in the files of each species, to see if a) the proteins had a species they
...
##I then subsetted the file to extract the names of the proteins, and only the ones with found matches in the species, performing a search of these names across the dataset containing
the files, each of each orthologue, searching for the protein, and if found extracting the protein and the file it was found in
